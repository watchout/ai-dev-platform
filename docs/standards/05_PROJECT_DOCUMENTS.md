...
## F. notes/ ディレクトリ（決定ログ・AI長期記憶）

### 目的

```
PRごとの意思決定ログと、AIの長期記憶を管理する。

問題:
  1. PRレビューのコメントやSlackの議論で決まった事項が
     散逸し、後から「なぜこう実装したか」が追えない。
  2. AI（Claude Code）は新しいセッションで過去の
     コンテキストを失い、同じ質問を繰り返す。

解決:
  notes/ ディレクトリに構造化された決定ログを蓄積し、
  人間とAIの両方が参照できる知識ベースにする。
```
...
docs/
└── notes/
    ├── decisions/          ← PR・レビューでの決定事項
    │   ├── 2026-02-03_auth-session-strategy.md
    │   ├── 2026-02-05_api-versioning-approach.md
    │   └── ...
    │
    ├── learnings/          ← プロジェクト固有の学習事項
    │   ├── 2026-02-03_supabase-rls-limitations.md
    │   ├── 2026-02-07_nextjs-middleware-gotchas.md
    │   └── ...
    │
    └── retrospectives/     ← 振り返り・改善記録
        ├── 2026-02-10_sprint-1-retro.md
        └── ...
```

### 決定ログ（decisions/）のフォーマット
...
## CLAUDE.md への反映
[CLAUDE.md に追記した内容（追記した場合）]
```

### AI長期記憶としての活用

```
CLAUDE.md に以下を追記して notes/ を参照させる:

## プロジェクト知識ベース
過去の意思決定や学習事項は docs/notes/ に蓄積されています。
実装前に関連する notes を確認してください。

特に以下の場面で参照すること:
- 同じ技術領域の実装をする時 → notes/learnings/ を確認
- 過去のPRと関連する機能を修正する時 → notes/decisions/ を確認
- エスカレーション（T1〜T7）が発生した時 → 過去に同様の決定がないか確認
```
...
1. PR マージ時に作成
   重要な決定を含むPRがマージされたら、
   決定ログを notes/decisions/ に追記する。

2. 問題解決時に作成
   技術的な問題を解決したら、
   学習ログを notes/learnings/ に追記する。

3. CLAUDE.md と連携
   notes/ に追記したら、必要に応じて
   CLAUDE.md の「学習済みルール」にも要約を追記する。

4. 命名規則
   YYYY-MM-DD_kebab-case-title.md

5. 検索性の確保
   各ノートのメタデータに関連SSOT、PR番号、
   カテゴリを記載して検索可能にする。
```
...
上記の notes/ ディレクトリは、以下のように全体構造に組み込まれる:
...
├── D. 運用ドキュメント
├── E. 管理ドキュメント
└── F. ナレッジベース（notes/）   ← 新規追加
    ├── F1. 決定ログ（decisions/）
    ├── F2. 学習ログ（learnings/）
    └── F3. 振り返り（retrospectives/）
```
...
| 日付 | 変更内容 | 変更者 |
|------|---------|-------|
| 2026-02-03 | F. notes/ディレクトリ（決定ログ・AI長期記憶）セクション追加 | AI |
| | 初版作成 | |
