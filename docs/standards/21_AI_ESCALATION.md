...
```
プロジェクトの学習内容を CLAUDE.md に自動追記し、
AI のコンテキスト品質を継続的に向上させる。

問題:
  新しいセッションを開始するたびに、AIは過去の
  学習内容（判断、制約、パターン）を忘れてしまう。

解決:
  各セッションで得た知見を CLAUDE.md に構造的に蓄積し、
  次のセッションで自動的に参照される仕組みにする。
```
...
## AI中断プロトコル（固定）
[21_AI_ESCALATION.md の要約]

## 学習済みルール（自動追記セクション）

### 技術的判断
- [日付] DB接続はコネクションプール必須（T3で確認済み）
- [日付] 日時はすべてUTC保存、表示時にJSTに変換（T4で解決）

### ビジネスルール
- [日付] 無料プランは月5回まで（T7で確認済み）
- [日付] 退会後30日間はデータ保持（T1で追加）

### コード規約の補足
- [日付] APIレスポンスは必ずcamelCaseに変換（T5で追加）
- [日付] テストデータはfactory関数で生成（T5で追加）

### 既知の制約
- [日付] Supabase のRLSはトリガー内で無効（T6で発見）
- [日付] Next.js App Routerではlayout.tsxで認証チェック（T3で決定）
```

### 追記タイミング

```
以下の場合に CLAUDE.md へ追記する:

1. エスカレーション解決後
   T1〜T7 の中断→質問→回答が得られたとき、
   その決定事項を「学習済みルール」に追記する。

2. SSOT 更新時
   SSOT を更新した場合、CLAUDE.md にも要約を追記する。
   （AIがSSOTの全文を毎回読まなくても判断できるように）

3. レビュー指摘の反映
   レビューで繰り返し指摘される内容を追記する。
   例: 「エラーメッセージは日本語で」→ 学習済みルールに追加
...
```markdown
### [カテゴリ]
- [YYYY-MM-DD] [内容]（[トリガーID]で[確認/追加/解決]）
```

### 追記のルール

```
1. 重複排除: 既に同じ内容が記載されていれば追記しない
2. 最新優先: 同じ項目の判断が変わった場合は上書き（旧版は削除）
3. 簡潔に: 1ルール1行。詳細はSSOTやADRを参照
4. カテゴリ分類: 技術的判断 / ビジネスルール / コード規約 / 既知の制約
5. 肥大化防止: 100行を超えたらカテゴリごとに外部ファイルに分離
   → .claude/rules/technical.md 等
```

### AI への指示（CLAUDE.md に記載）

```markdown
## CLAUDE.md 自動更新ルール

以下の場合、このファイルの「学習済みルール」セクションに追記してください:

1. エスカレーション（T1〜T7）が解決されたとき
2. SSOTが更新されたとき
3. 新しい技術的制約が判明したとき
...
| 日付 | 変更内容 | 変更者 |
|------|---------|-------|
| 2026-02-03 | 自己進化型CLAUDE.mdセクション追加 | AI |
| | 初版作成 | |
